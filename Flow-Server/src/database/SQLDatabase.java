...
public class SQLDatabase {
	...
	*/
	...
	protected boolean authenticate(String username, String password) {
		try {
			ResultSet pair = this.query("select password from users where username = " + username + ";");
			if(pair.next()) {
				// TODO Verify
	 ...
	 * <ul><li>The selected username already exists in the database</li>
	 * <li>An error was thrown when searching the database for all current users
	 * </li><li>An error was thrown when inserting user into the database</li>
	 * </ul>
	 */
	protected boolean addUser(String username, String password) {
		try {
			// Checks if a user with the specified username already exsists
			if(this.query("select username from users where username = " + username + ";").next()) {
				return false;
			}
		} catch (SQLException e) {
		...
		try {
			this.update("insert into users (username, password) values (" + username + ", " + password + ");");
			e.printStackTrace();
			return false;
		}
		return true;
	...
	/**
	* Internal method which calls the '{@link Statement#ExecuteQuery} method with 
	* the specified query.
	* 
	...
	*/
	...
	/**
	* Internal method which calls the '{@link Statement#ExecuteUpdate} method with 
	* the specified query.
	* 
