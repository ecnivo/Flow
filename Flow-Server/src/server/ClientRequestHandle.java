        ...
        try {
            ...
            switch (data.getType()) {
                ...
                case "new_project":
                    ...
                    break;
                case "project_modify":
                    String projectId = data.get("project_uuid", UUID.class).toString();
                    switch (data.get("project_modify_type", String.class)) {
                        case "MODIFY_COLLABORATOR":
                            ...
                            try {
                                ...
                                returnData.put("status", this.database.updateAccess((int) data.get("access_level", Byte.class), projectId, username));
                            } catch (DatabaseException e) {
                                e.printStackTrace();
                                returnData.put("status", e.getMessage());
                            } catch (SQLException e) {
                        ...
                        case "RENAME_PROJECT":
                            ...
                            break;
                        case "DELETE_PROJECT":
                            returnData.put("status", this.database.deleteProject(projectId));
                            break;
                    }
                    break;
                case "new_textdocument": {
                    UUID projectUUID = UUID.fromString(data.get("project_uuid", String.class));
                    UUID directoryUUID = UUID.fromString(data.get("directory_uuid", String.class));
                    ...
                    DataManagement.getInstance().addTextDocument(fileUUID, newTextDocument);
                }
                break;
                case "new_directory": {
                    UUID projectUUID = data.get("project_uuid", UUID.class);
                ...
                case "project_info":
                    try {
                        ...
                        returnData.put("owner", this.database.getUsers(projectUUID.toString(), SQLDatabase.OWNER));
                        returnData.put("status", "OK");
                    } catch (DatabaseException e) {
                        e.printStackTrace();
                        returnData.put("status", e.getMessage());
                ...
                case "directory_info":
                    try {
                        ...
                        UUID projectUUID = data.get("project_uuid", UUID.class),
                                directoryUUID = data.get("directory_uuid", UUID.class);
                        ...
                        // specified directory
                        results = this.database.getFiles(projectUUID.toString(), directoryUUID.toString());
                        returnData.put("child_files", DataModification.getUUIDsFromArray(Results.toStringArray("DocumentID", results)));

                        // Add information from all sub directories located inside
                        // the specified directory
                        results = this.database.getDirectories(projectUUID.toString(), directoryUUID.toString());
                        returnData.put("child_directories", DataModification.getUUIDsFromArray(Results.toStringArray("DirectoryID", results)));
				 ...
				 */
                    try {
                        ...
                        returnData.put("file_type", results.getString("DocumentType"));
                        returnData.put("status", "OK");
                    } catch (DatabaseException e) {
                        e.printStackTrace();
                        L.severe(e.getMessage());
                ...
                case "file_versions":
                    try {
                        UUID fileUUID = data.get("file_uuid", UUID.class);
                        returnData.put("file_versions", DataModification.getUUIDsFromArray(this.database.getFileVersions(fileUUID.toString())));
                        returnData.put("status", "OK");
                    } catch (DatabaseException e) {
                        e.printStackTrace();
                        returnData.put("status", e.getMessage());
                ...
                case "end_session":
                    ...
                    this.database.removeSession(data.get("session_id", UUID.class).toString());
                    break;
                default:
