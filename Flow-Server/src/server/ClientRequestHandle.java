...
public class ClientRequestHandle implements Runnable {
	...
	try {
		...
		case "CLOSE_ACCOUNT":
		    try {
			String status = "OK", username = this.database.getUsername(data.get("session_id", UUID.class).toString());
		...
		try {
			...
			DataManagement.getInstance().addTextDocumentVersion(fileUUID, versionUUID, newTextDocument);
		...
		case "MODIFY_COLLABORATOR":
		    ...
		    try {
			sessionInfo = this.database.getSessionInfo(data.get("session_id"));
			sessionInfo.next();
		...
		try {
		    ...
		    ResultSet results = this.database.getFileInfo(fileUUID.toString());
		    returnData.put("file_name", results.getString("DocumentName"));
		    returnData.put("file_type", results.getString("FileType"));
		    returnData.put("file_versions", DataManipulation.getUUIDsFromArray(this.database.getFileVersions(fileUUID.toString())));
		    returnData.put("status", "OK");
		} catch (DatabaseException e) {
		    e.printStackTrace();
		    L.severe(e.getMessage());
		...
		try {
		    returnData.put("status", this.database.getVersionDate(versionUUID.toString()));
		    returnData.put("date", (long) 100); // This should be the
							// number of millis
							// since Epoch
		    // TODO Bimde add your millis thing
		} catch (DatabaseException e) {
		    e.printStackTrace();
		    returnData.put("status", e.getMessage());
		}
		...
		try {
			...
			VersionText doc = DataManagement.getInstance().getTextDocument(fileUUID, versionUUID);
		...
		try {
		    ...
		    VersionText td = DataManagement.getInstance().getTextDocument(fileUUID, latestVersionUUID);
