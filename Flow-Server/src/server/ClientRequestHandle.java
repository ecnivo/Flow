...
public class ClientRequestHandle implements Runnable {
    ...
    public ClientRequestHandle(FlowServer server, Socket socket) throws IOException {
        this.socket = socket;
    ...
    public void run() {
        try {
            ...
            switch (data.getType()) {
                case "login":
                    try {
                        String username = data.get("username", String.class), password = data.get("password", String.class);
                ...
                case "user":
                    ...
                    switch (userCmdType) {
                        case "REGISTER":
                            returnData.put("status", this.database.addUser(data.get("username", String.class), data.get("password", String.class)));
		     ...
		     */
                        ...
                        case "CHANGE_PASSWORD":
                            try {
                                this.database.changePassword(this.database.getUsername(data.get("session_id", UUID.class).toString()), data.get("new_password", String.class));
                                returnData.put("status", "OK");
                            } catch (DatabaseException e) {
                                e.printStackTrace();
                                returnData.put("status", e.getMessage());
                ...
                case "new_text_file":
                    try {
                        UUID projectUUID = data.get("project_uuid", UUID.class), sessionID = data.get("session_id", UUID.class);
                        if (this.database.verifyPermissions(sessionID.toString(), projectUUID.toString())) {
                            UUID directoryUUID = data.get("directory_uuid", UUID.class), fileUUID = UUID.randomUUID(), versionUUID = UUID.randomUUID();
                ...
                case "directory_modify": {
                    try {
                        ...
                        String sessionID = data.get("session_id", UUID.class).toString(), projectUUID = this.database.getProjectUUIDFromDirectory(directoryUUID.toString());
                        ...
                        if (this.database.verifyPermissions(sessionID, projectUUID)) {
                            ...
                            switch (type) {
                                case "RENAME":
                                    String newName = data.get("new_name", String.class);
                                    returnData.put("status", this.database.renameDirectory(directoryUUID.toString(), newName));
                                    break;
                                case "DELETE":
                                    returnData.put("status", this.database.deleteDirectory(directoryUUID.toString()));
                                    break;
                            }
                        } else {
                            returnData.put("status", "INVALID_SESSION_ID");
                ...
                case "file_metadata_modify":
                    try {
                        ...
                        String sessionID = data.get("session_id", UUID.class).toString(), projectUUID = this.database.getProjectUUIDFromFile(fileUUID.toString());
                        if (this.database.verifyPermissions(sessionID, projectUUID)) {
                            String modType = data.get("mod_type", String.class);
                            switch (modType) {
                                case "RENAME":
                                    returnData.put("status", this.database.renameFile(fileUUID.toString(), data.get("new_name", String.class)));
                                    break;
                                case "DELETE":
                                    DataManagement.getInstance().removeFileByUUID(fileUUID);
                                    returnData.put("status", this.database.deleteFile(fileUUID.toString()));
                                    break;
                            }
                        } else {
                            returnData.put("status", "INVALID_SESSION_ID");
                ...
                case "project_info":
                    try {
                        ...
                        if (this.database.verifyPermissions(sessionID, projectUUID.toString())) {
                            ...
                            returnData.put("editors", this.database.getUsers(projectUUID.toString(), SQLDatabase.EDIT));
                            returnData.put("viewers", this.database.getUsers(projectUUID.toString(), SQLDatabase.VIEW));
                            returnData.put("owner", this.database.getUsers(projectUUID.toString(), SQLDatabase.OWNER)[0]);
                            returnData.put("status", "OK");
                        } else {
                            returnData.put("status", "INVALID_SESSION_ID");
                        }
                ...
                case "directory_info":
                    try {
                        ...
                        if (this.database.verifyPermissions(sessionID, projectUUID)) {
                            ...
                            ResultSet results = this.database.getDirectoryInfo(directoryUUID.toString());
                            returnData.put("parent_directory_uuid", results.getString("ParentDirectoryID"));
                            returnData.put("directory_name", results.getString("DirectoryName"));

                            // Add information from all documents located in the
                            // specified directory
                            results = this.database.getFilesInDirectory(directoryUUID.toString());
                            returnData.put("child_files", DataManipulation.getUUIDsFromArray(Results.toStringArray("DocumentID", results)));
                            ...
                            returnData.put("child_directories", DataManipulation.getUUIDsFromArray(Results.toStringArray("DirectoryID", results)));
                ...
                case "file_info":
                    try {
                        // Load the required data from the data packet
                        String fileUUID = data.get("file_uuid", UUID.class).toString(), sessionID = data.get("session_id", UUID.class).toString(), projectUUID = this.database.getProjectUUIDFromFile(fileUUID);

                        // Verify is the user has at least view access
                        if (this.database.verifyPermissions(sessionID, projectUUID)) {
                            // Add information from the Documents table
                            ResultSet results = this.database.getFileInfo(fileUUID);
                            returnData.put("file_name", results.getString("DocumentName"));
                            returnData.put("file_type", results.getString("FileType"));
                            returnData.put("file_versions", DataManipulation.getUUIDsFromArray(this.database.getFileVersions(fileUUID)));
                            returnData.put("status", "OK");
                        } else {
                            returnData.put("status", "INVALID_SESSION_ID");
                        }
                ...
                case "version_info":
                    try {
                        String versionUUID = data.get("version_uuid", UUID.class).toString(), sessionID = data.get("session_id", UUID.class).toString(), projectUUID = this.database.getProjectUUIDFromVersion(versionUUID);
                ...
                case "request_version":
                    try {
                        UUID fileUUID = data.get("file_uuid", UUID.class), versionUUID = data.get("version_uuid", UUID.class);
                        String sessionID = data.get("session_id", UUID.class).toString(), projectUUID = this.database.getProjectUUIDFromFile(fileUUID.toString());
                        if (this.database.verifyPermissions(sessionID, projectUUID)) {
                            byte[] bytes = null;
                ...
                case "file_request":
                    try {
                        ...
                        String sessionID = data.get("session_id", UUID.class).toString(), projectUUID = this.database.getProjectUUIDFromFile(fileUUID.toString());
                        if (this.database.verifyPermissions(sessionID, projectUUID)) {
                            UUID versionUUID = UUID.fromString(this.database.getLatestVersionUUID(fileUUID.toString()));
                    ...
                    try {
                        ...
                        switch (data.get("mod_type", String.class)) {
                            case "INSERT": {
                                ...
                                DocumentCallbackEvent event = new DocumentCallbackEvent(DocumentCallbackEvent.DocumentCallbackType.INSERT, fileUUID, username, idx, str, -1);
                                PersistentHandleManager.getInstance().doCallbackEvent(fileUUID, event);
                            }
                            break;
                            case "DELETE": {
                                ...
                                while (len-- > 0)
                                    td.delete(idx);
                                DocumentCallbackEvent event = new DocumentCallbackEvent(DocumentCallbackEvent.DocumentCallbackType.DELETE, fileUUID, username, idx, null, len);
                                PersistentHandleManager.getInstance().doCallbackEvent(fileUUID, event);
                            }
                            break;
                        }
